...
public class userServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        switch (strFormId) 
        {
            case "1":
                ...
                if(bValidUser) 
                {
                    if (userObjects!=null)
                    {
                        ...
                        else if(userObjects.getType().equals("root"))
                        {
                            request.getSession().setAttribute("current_user", userObjects);
                            request.getSession().removeAttribute("message");
                            response.sendRedirect("root_dash.jsp");
                            
                        }else if (userObjects.getType().equals("repartidor")) {
                            request.getSession().setAttribute("current_user", userObjects);
                            request.getSession().removeAttribute("message");
                            response.sendRedirect("repar_dash.jsp");
                        }
                    }
                }
                else
            ...
            break;
            case "9":
                //Borrar usuarios por parte del root
